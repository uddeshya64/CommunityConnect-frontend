  ...
  useEffect(() => {
    const verifyTokenAndAuth = async () => {
      try {
        ...
        if (authToken) {
          setIsLoggedIn(true);
          // Extract email from JWT to verify it matches the invite
          try {
        ...
        // 2. Verify Invite Token (Public Route)
        // MATCHED: GET /api/team-dashboard/verify-invite/:token
        const response = await api.get(`/team-dashboard/verify-invite/${token}`);
        
        // Handle your backend's { success: true, data: { ... } } wrapper
    ...
    if (token) verifyTokenAndAuth();
  }, [token]);

  const handleAcceptInvite = async () => {
    if (!isLoggedIn) {
      // If not logged in, redirect to auth, but save the invite URL so they return here after logging in
    ...
    try {
      ...
      // MATCHED: POST /api/team-dashboard/accept-invite
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#09090b] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <p className="text-zinc-400 font-bold uppercase tracking-[0.2em] text-sm relative z-10">Verifying Invitation...</p>
      </div>
    );
  }
