    ...
    <div className={`min-h-screen ${isDark ? "bg-zinc-950 text-zinc-100" : "bg-zinc-50 text-zinc-900"} relative overflow-hidden transition-colors duration-300`}>
      ...
      <nav className="relative z-10 flex items-center justify-between p-6 max-w-6xl mx-auto">
        ...
        <div className="space-x-4">
          <Link href="/login" className={`text-sm font-semibold ${isDark ? "text-zinc-400 hover:text-white" : "text-zinc-600 hover:text-zinc-900"} transition-colors`}>
            Log in
          </Link>
          <Link href="/register">
            <Button className={`rounded-full ${activeAccent.bg} text-white hover:opacity-90 shadow-sm px-6`}>
              Sign up
            </Button>
          </Link>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center text-center px-4 pt-32 pb-20 max-w-4xl mx-auto space-y-8">
        ...
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${isDark ? "bg-zinc-900/80 border-white/10 text-zinc-300" : "bg-white border-zinc-200 text-zinc-600"} shadow-sm text-sm font-medium`}>
          <span className={`flex h-2 w-2 rounded-full ${activeAccent.bg}`}></span>
          Discover events near you
        </div>

        <h1 className={`text-5xl md:text-7xl font-extrabold tracking-tighter ${isDark ? "text-white" : "text-zinc-900"}`}>
        ...
        <p className={`text-lg md:text-xl ${isDark ? "text-zinc-400" : "text-zinc-500"} max-w-2xl font-medium`}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-8 w-full sm:w-auto">
          <Link href="/events" className="w-full sm:w-auto">
            <Button size="lg" className={`w-full rounded-full ${activeAccent.bg} text-white hover:opacity-90 shadow-md ${activeAccent.shadow} px-8 py-6 text-lg font-semibold transition-all hover:scale-105`}>
              Explore Events
            </Button>
          </Link>
          <Link href="/register" className="w-full sm:w-auto">
            <Button variant="outline" size="lg" className={`w-full rounded-full px-8 py-6 text-lg font-semibold ${isDark ? "border-white/10 bg-zinc-900/60 hover:bg-zinc-800 text-zinc-200" : "border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700"} shadow-sm transition-all hover:scale-105`}>
              Host an Event
            </Button>
          </Link>
        </div>
