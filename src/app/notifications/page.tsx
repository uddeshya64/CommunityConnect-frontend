          ...
          <div className="mb-10 flex items-center justify-between">
            <div>
              ...
              <p className={`font-medium text-sm sm:text-base ${
                ...
                Manage your invitations, role updates, and live event quizzes.
              </p>
            </div>
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className={`text-xs font-medium ${isDark ? "text-zinc-400" : "text-zinc-500"} mt-0.5`}>
                    Get WhatsApp-style alerts on your phone or PC for live quizzes, invites, and updates.
                  </p>
                </div>
              </div>
          ...
          {!isLoading && notifications.length === 0 && (
            ...
            >
              ...
              <p className={`font-medium max-w-sm mb-8 ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>
                You have no active notifications or live quiz alerts. Join events or teams to participate!
              </p>
              <Link href="/home">
                ...
                {notifications.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {item.eventBanner ? (
                        ...
                        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${
                          ...
                          {item.type === 'LIVE_QUIZ' ? <Zap className="w-7 h-7" /> : item.type === 'TEAM_INVITE' ? <Users className="w-7 h-7" /> : <Shield className="w-7 h-7" />}
                        </div>
                      )}
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                            ...
                            {item.type === 'LIVE_QUIZ' ? 'Live Quiz 🚀' : item.type === 'TEAM_INVITE' ? 'Team Invite' : item.type === 'ROLE_UPDATE' ? 'Role Updated' : 'Staff Invite'}
                          </span>
                        ...
                        {item.type === 'LIVE_QUIZ' ? (
                          <p className={`font-semibold text-base leading-snug ${isDark ? "text-zinc-200" : "text-zinc-800"}`}>
                            Quiz <span className={`font-black ${activeAccent.text}`}>{item.quizTitle || 'Live Quiz'}</span> is now LIVE for event <span className={`font-bold ${isDark ? "text-white" : "text-zinc-950"}`}>{item.eventName}</span>!
                          </p>
                        ) : item.type === 'TEAM_INVITE' ? (
                          <p className={`font-semibold text-base leading-snug ${isDark ? "text-zinc-200" : "text-zinc-800"}`}>
                    ...
                    <div className="flex items-center gap-3 shrink-0 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 justify-end">
                      {item.type === 'LIVE_QUIZ' ? (
                        <Link href={`/events/${item.eventId}/quiz/${item.quizId}`}>
                          <Button className="rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-extrabold px-6 h-10 shadow-lg hover:brightness-110 transition-all hover:scale-105">
                            🎯 Join Quiz Now
                          </Button>
                        </Link>
                      ) : item.type === 'ROLE_UPDATE' ? (
